...
# UNKNOWN が返ることが期待値。
#
# WR-06: プロンプトでファイル読み取り・検索・シェル実行を明示的に禁止しているため、
# この対照実験で 7331 が出現することは「ファイル検索由来」では説明できない。それは
# (a) ファイル検索禁止制約が守られていない、または (b) 非 fresh で会話継続性が存在する、
# のいずれかを意味し、どちらも段階 3 の方法論的前提（非 fresh では継続性がない）を破壊する。
# 前提が崩れれば段階 3 の「7331 非出現」サブチェックは何も証明しない。よって 7331 の出現は
# WARNING ではなくハード FAIL とする（対照実験が黙って PASS して前提崩壊を隠すのを防ぐ）。
#
...
# WR-06: 7331 の出現は対照実験の前提を破壊するためハード FAIL とする（先に判定）
if echo "$result25" | grep -q "7331"; then
    log "ERROR: 段階 2.5 失敗 — 対照実験の result に 7331 が含まれる"
    log "  ファイル検索禁止プロンプト下での 7331 出現は、ファイル検索禁止が守られていないか"
    log "  非 fresh での会話継続性の存在を意味し、いずれも段階 3 の方法論的前提を破壊する。"
    log "  result25: ${result25:0:300}"
    exit 1
fi
